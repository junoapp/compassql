...
const DEFAULT_SPEC_CONFIG = config_1.DEFAULT_QUERY_CONFIG.defaultSpecConfig;
describe('SpecQueryModel', () => {
    ...
    describe('build', () => {
        ...
        it('should have mark wildcardIndex if mark is a ShortWildcard.', () => {
            ...
            const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
            chai_1.assert.deepEqual(wildcardIndex.mark, {
                name: 'm',
                enum: config_1.DEFAULT_QUERY_CONFIG.enum.mark
            });
        });
        ...
        it('should have no mark wildcardIndex if mark is specific.', () => {
            ...
            const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
            chai_1.assert.isNotOk(wildcardIndex.mark);
        });
        // TODO: Transform
        // Encoding
        describe('type', () => {
            it('should automatically add type as wildcard and index it', () => {
                ...
                const specM = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG);
                // check if type is wildcard
                chai_1.assert.isTrue(wildcard_1.isWildcard(specM.getEncodingQueryByIndex(0).type));
                // check if enumeration specifier index has an index for type
                chai_1.assert.isOk(specM.wildcardIndex.encodings[0].get('type'));
            });
        });
        const templateSpecQ = {
            mark: MARK.POINT,
        ...
        property_1.ENCODING_TOPLEVEL_PROPS.forEach(prop => {
            it(`should have ${prop} wildcardIndex if it is a ShortWildcard.`, () => {
                let specQ = util_1.duplicate(templateSpecQ);
                // set to a short wildcard
                specQ.encodings[0][prop] = wildcard_1.SHORT_WILDCARD;
                const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
                chai_1.assert.isOk(wildcardIndex.encodingIndicesByProperty.get(prop));
                chai_1.assert.isOk(wildcardIndex.encodings[0].get(prop));
            });
            it(`should have ${prop} wildcardIndex if it is an Wildcard.`, () => {
                let specQ = util_1.duplicate(templateSpecQ);
                // set to a full wildcard
                const enumValues = prop === property_1.Property.FIELD ? ['A', 'B'] : wildcard_1.getDefaultEnumValues(prop, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG);
            ...
            it(`should not have ${prop} wildcardIndex if it is specific.`, () => {
                ...
                const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
                chai_1.assert.isNotOk(wildcardIndex.encodingIndicesByProperty.get(prop));
                chai_1.assert.isNotOk(wildcardIndex.encodings[0]);
            });
        });
        property_1.ENCODING_NESTED_PROPS.forEach(nestedProp => {
            const propKey = property_1.toKey(nestedProp);
            ...
            it(`should have ${propKey} wildcardIndex if it is a ShortWildcard.`, () => {
                ...
                specQ.encodings[0][parent][child] = wildcard_1.SHORT_WILDCARD;
                const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
                chai_1.assert.isOk(wildcardIndex.encodingIndicesByProperty.get(nestedProp));
                chai_1.assert.isOk(wildcardIndex.encodings[0].get(nestedProp));
            });
            it(`should have ${propKey} wildcardIndex if it is an Wildcard.`, () => {
                let specQ = util_1.duplicate(templateSpecQ);
                specQ.encodings[0][parent] = {};
                specQ.encodings[0][parent][child] = {
                    enum: wildcard_1.getDefaultEnumValues(nestedProp, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG)
                };
                const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
                chai_1.assert.isOk(wildcardIndex.encodingIndicesByProperty.get(nestedProp));
                chai_1.assert.isOk(wildcardIndex.encodings[0].get(nestedProp));
            ...
            it(`should not have ${propKey} wildcardIndex if it is specific.`, () => {
                let specQ = util_1.duplicate(templateSpecQ);
                const wildcardIndex = model_1.SpecQueryModel.build(specQ, fixture_1.schema, config_1.DEFAULT_QUERY_CONFIG).wildcardIndex;
                chai_1.assert.isNotOk(wildcardIndex.encodingIndicesByProperty.get(nestedProp));
                chai_1.assert.isNotOk(wildcardIndex.encodings[0]);
            });
        });
        describe('autoCount', () => {
            ...
            const model = model_1.SpecQueryModel.build(specQ, fixture_1.schema, util_1.extend({}, config_1.DEFAULT_QUERY_CONFIG, { autoAddCount: true }));
            it('should add new encoding if autoCount is enabled', () => {
                chai_1.assert.equal(model.specQuery.encodings.length, 2);
                chai_1.assert.isTrue(wildcard_1.isWildcard(model.specQuery.encodings[1].autoCount));
            });
            it('should add new channel and autoCount to the wildcard', () => {
                chai_1.assert.equal(model.wildcardIndex.encodingIndicesByProperty.get('autoCount')[0], 1);
                chai_1.assert.equal(model.wildcardIndex.encodingIndicesByProperty.get('channel')[0], 1);
            });
        });
    });
    describe('channelUsed', () => {
        it('should return true if channel is used for general fieldDef', () => {
            ...
            chai_1.assert.isTrue(specM.channelUsed(CHANNEL.X));
        });
        it('should return false if channel is used for general fieldDef', () => {
            ...
            chai_1.assert.isFalse(specM.channelUsed(CHANNEL.Y));
        });
        it('should return false if channel is used for disabled autoCount', () => {
            ...
            chai_1.assert.isFalse(specM.channelUsed(CHANNEL.X));
        });
    });
    describe('isAggregate', () => {
        it('should return false if the query is not aggregated', () => {
            ...
            chai_1.assert.isFalse(specM.isAggregate());
        });
        it('should return true if the query is aggregated', () => {
            ...
            chai_1.assert.isTrue(specM.isAggregate());
        });
    ...
    describe('getEncodings()', () => {
        it('should return correct encodings', () => {
            ...
            chai_1.assert.deepEqual(specM.getEncodings(), [
                { channel: CHANNEL.Y, value: 5 },
                { channel: CHANNEL.COLOR, field: 'a', type: 'quantitative' }
            ]);
        });
    ...
    describe('toSpec', () => {
        ...
        it('should return a correct Vega-Lite spec if the query has autoCount=false even if channel is unspecified', () => {
            ...
            const specM = buildSpecQueryModel({
                mark: MARK.BAR,
                encodings: [{ channel: CHANNEL.X, field: 'O', type: TYPE.ORDINAL }, { channel: wildcard_1.SHORT_WILDCARD, autoCount: false }]
            });
            const spec = specM.toSpec();
            chai_1.assert.deepEqual(spec, {
                mark: MARK.BAR,
        ...
        it('should return null if the query is incompleted', () => {
            ...
            chai_1.assert.isNull(specM.toSpec());
        });
        ...
        it('should return a spec with width and height specified', () => {
            ...
            chai_1.assert.equal(specM.toSpec().width, 100);
            chai_1.assert.equal(specM.toSpec().height, 120);
        });
        it('should return a spec with background color as specified', () => {
            ...
            chai_1.assert.equal(specM.toSpec().background, 'black');
        });
        it('should return a spec with padding as specified', () => {
            ...
            chai_1.assert.deepEqual(specM.toSpec().padding, { left: 1, top: 2, right: 3, bottom: 4 });
        });
        it('should return a spec with a title string specified', () => {
            ...
            chai_1.assert.equal(specM.toSpec().title, 'Big Title');
        });
        it('should return a spec with a title params specified', () => {
            ...
            chai_1.assert.equal(title.text, 'A Simple Bar Chart');
            chai_1.assert.equal(title.anchor, 'start');
        });
    });
});
describe('SpecQueryModelGroup', () => {
    describe('constructor', () => {
        ...
        it('should have default values', () => {
            chai_1.assert.equal(group.name, '');
            chai_1.assert.equal(group.path, '');
            chai_1.assert.isArray(group.items);
            chai_1.assert.deepEqual(group.items, []);
            chai_1.assert.equal(group.groupBy, undefined);
            chai_1.assert.equal(group.orderGroupBy, undefined);
        });
    });
});
