...
const spec_1 = require("../../src/constraint/spec");
...
describe('constraints/spec', () => {
    const schema = new schema_1.Schema({ fields: [] });
    ...
    spec_1.SPEC_CONSTRAINTS.forEach(constraint => {
    ...
    describe('alwaysIncludeZeroInScaleWithBarMark', () => {
        it('should return false if scale does not start at zero when bar mark is used', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['alwaysIncludeZeroInScaleWithBarMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return true if scale starts at zero when bar mark is used', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['alwaysIncludeZeroInScaleWithBarMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('hasAllRequiredPropertiesSpecific', () => {
        const specCModel = new spec_1.SpecConstraintModel({
            ...
            properties: [property_1.Property.AGGREGATE, property_1.Property.TYPE, property_1.Property.SCALE, { parent: 'scale', child: 'type' }, property_1.Property.MARK],
        ...
        it('should return true if all properties is defined', () => {
            const specQM = buildSpecQueryModel({
                mark: MARK.POINT,
                encodings: [
                    { aggregate: 'mean', channel: CHANNEL.X, field: 'A', scale: { type: scale_1.ScaleType.LOG }, type: TYPE.QUANTITATIVE }
                ]
            });
            chai_1.assert.isTrue(specCModel.hasAllRequiredPropertiesSpecific(specQM));
        });
        ...
        it('should return false if a required property is a wildcard', () => {
            const specQM = buildSpecQueryModel({
                mark: MARK.POINT,
                encodings: [{ aggregate: 'mean', channel: CHANNEL.X, field: 'A', scale: wildcard_1.SHORT_WILDCARD, type: TYPE.QUANTITATIVE }]
            });
            chai_1.assert.isFalse(specCModel.hasAllRequiredPropertiesSpecific(specQM));
        });
        it('should return false if a nested required property is a wildcard', () => {
            const specQM = buildSpecQueryModel({
                mark: MARK.POINT,
                encodings: [
                    { aggregate: 'mean', channel: CHANNEL.X, field: 'A', scale: { type: wildcard_1.SHORT_WILDCARD }, type: TYPE.QUANTITATIVE }
                ]
            });
            chai_1.assert.isFalse(specCModel.hasAllRequiredPropertiesSpecific(specQM));
        });
    ...
    describe('autoAddCount', () => {
        function autoCountShouldBe(expectedAutoCount, when, baseSpecQ) {
            [true, false].forEach(satisfy => {
                ...
                it(`should return ${satisfy} when autoCount is ${autoCount} and ${when}.`, () => {
                    const specQ = util_1.duplicate(baseSpecQ);
                    const model = model_1.SpecQueryModel.build(specQ, schema, Object.assign(Object.assign({}, config_1.DEFAULT_QUERY_CONFIG), { autoAddCount: true }));
                    ...
                    chai_1.assert.equal(spec_1.SPEC_CONSTRAINT_INDEX['autoAddCount'].satisfy(model, schema, config_1.DEFAULT_QUERY_CONFIG), satisfy);
                });
            });
        }
        ...
        it('should return true for a raw plot that has only an unbinned quantitative field and a temporal field without time unit', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['autoAddCount'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        ...
        it('should return false if the raw spec have only nominal, ordinal, binned quantitative or temporal with time unit fields.', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['autoAddCount'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('channelPermittedByMarkType', () => {
        it('should return true for supported channel', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['channelPermittedByMarkType'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false for unsupported channel', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['channelPermittedByMarkType'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('hasAllRequiredChannelsForMark', () => {
        it('should return true if area/line have both x and y', () => {
            [MARK.AREA, MARK.LINE].forEach(mark => {
                ...
                chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['hasAllRequiredChannelsForMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
        it('should return false if area/line have do not have x or y', () => {
            [CHANNEL.X, CHANNEL.Y, CHANNEL.COLOR].forEach(channel => {
                [MARK.AREA, MARK.LINE].forEach(mark => {
                    ...
                    chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['hasAllRequiredChannelsForMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
                });
            });
        });
        ...
        describe('bar, tick', () => {
            it('should return true for plots with one dimension and one measure on x/y', () => {
                [MARK.BAR, MARK.TICK].forEach(mark => {
                    ...
                    chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['hasAppropriateGraphicTypeForMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
                });
            });
            ...
            it('should return false for plots with two dimensions on x/y', () => {
                [MARK.BAR, MARK.TICK].forEach(mark => {
                    ...
                    chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['hasAppropriateGraphicTypeForMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
                });
            });
            ...
            it('should return true for aggregate line/area with one temporal field', () => {
                [MARK.LINE, MARK.AREA].forEach(mark => {
                    [undefined, 'year'].forEach((timeUnit) => {
                        ...
                        chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['hasAppropriateGraphicTypeForMark'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG), `${mark}, ${timeUnit} test`);
                    });
                });
            });
    ...
    describe('noRepeatedChannel', () => {
        it('should return true when there is no repeated channels', function () {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['noRepeatedChannel'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false when there are repeated channels', function () {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['noRepeatedChannel'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('omitAggregate', () => {
        it('should return true if there is only raw data', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregate'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false if there is aggregate data', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregate'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('omitAggregatePlotWithDimensionOnlyOnFacet', () => {
        it('should return false if the only dimension is facet and it is enumerated', () => {
            ...
            specM.setEncodingProperty(2, property_1.Property.CHANNEL, CHANNEL.ROW, { enum: [CHANNEL.ROW] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregatePlotWithDimensionOnlyOnFacet'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return true if the only dimension is facet and it is specified', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregatePlotWithDimensionOnlyOnFacet'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false if the only dimension is facet and it is specified when we constraintManuallySpecifiedValue', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregatePlotWithDimensionOnlyOnFacet'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
        });
    ...
    describe('omitAggregatePlotWithoutDimension', () => {
        it('should return false if plot is aggregate and has no dimension', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregatePlotWithoutDimension'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return true if plot is aggregate and has dimension', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitAggregatePlotWithoutDimension'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    ...
    describe('omitBarLineAreaWithOcclusion', () => {
        [MARK.BAR, MARK.LINE, MARK.AREA].forEach(mark => {
            it(`should return false for raw ${mark}`, () => {
                ...
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitBarLineAreaWithOcclusion'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
            it(`should return true for aggregate ${mark}`, () => {
                ...
                chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitBarLineAreaWithOcclusion'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
    ...
    describe('omitBarTickWithSize', () => {
        it('should return false if bar/tick enumerates size', () => {
            [MARK.BAR, MARK.TICK].forEach(mark => {
                ...
                specM.setEncodingProperty(2, property_1.Property.CHANNEL, CHANNEL.SIZE, { enum: [CHANNEL.SIZE] });
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitBarTickWithSize'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
        it('should return true if bar/tick contains manually specified size', () => {
            [MARK.BAR, MARK.TICK].forEach(mark => {
                ...
                chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitBarTickWithSize'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
        it('should return false if bar/tick contains manually specified size and we constraintManuallySpecifiedValue', () => {
            [MARK.BAR, MARK.TICK].forEach(mark => {
                ...
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitBarTickWithSize'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
            });
        });
    ...
    describe('omitBarAreaForLogScale', () => {
        it('should return false if either x or y channel of bar or area mark is log scale', () => {
            const specM = buildSpecQueryModel({
                mark: MARK.AREA,
                encodings: [
                    { channel: CHANNEL.X, field: 'A', scale: { type: scale_1.ScaleType.LOG }, type: TYPE.QUANTITATIVE },
                    { channel: CHANNEL.Y, field: 'B', type: TYPE.QUANTITATIVE }
                ]
            });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitBarAreaForLogScale'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return true if either x or y channel of bar or area mark is not log scale', () => {
            const specM = buildSpecQueryModel({
                mark: MARK.AREA,
                encodings: [
                    { channel: CHANNEL.COLOR, field: 'A', scale: { type: scale_1.ScaleType.LOG }, type: TYPE.QUANTITATIVE },
                    { channel: CHANNEL.SHAPE, field: 'B', type: TYPE.QUANTITATIVE }
                ]
            });
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitBarAreaForLogScale'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('omitMultipleNonPositionalChannels', () => {
        it('should return true if there are zero non-positional channels', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitMultipleNonPositionalChannels'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        ...
        it('should return false if there are multiple non-positional channels and at least one of them is enumerated', () => {
            ...
            specM.setEncodingProperty(2, property_1.Property.CHANNEL, CHANNEL.SHAPE, { enum: [CHANNEL.SHAPE, CHANNEL.SIZE] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitMultipleNonPositionalChannels'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        ...
        it('should return false if there are multiple non-positional channels and we constraintManuallySpecifiedValue', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitMultipleNonPositionalChannels'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
        });
    ...
    describe('omitInvalidStackSpec', () => {
        const NON_NULL_STACK_OFFSETS = util_1.without(wildcard_1.DEFAULT_ENUM_INDEX.stack, [null]);
        it('should return false for stack with non linear scale type', () => {
            [
                scale_1.ScaleType.LOG,
                scale_1.ScaleType.POINT,
                scale_1.ScaleType.BAND,
                scale_1.ScaleType.POINT,
                ...
                NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                    ...
                    specM.wildcardIndex.setEncodingProperty(0, property_1.Property.STACK, { name: 'stack', enum: wildcard_1.DEFAULT_ENUM_INDEX.stack });
                    chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitInvalidStackSpec'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG), `for ${scaleType}`);
                });
            });
        });
        it('should return true for stack with linear scale type', () => {
            NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                ...
                specM.wildcardIndex.setEncodingProperty(0, property_1.Property.STACK, { name: 'stack', enum: wildcard_1.DEFAULT_ENUM_INDEX.stack });
                chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitInvalidStackSpec'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
            ...
            NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                const specM = buildSpecQueryModel({
                    ...
                    encodings: [
                        ...
                        { channel: CHANNEL.Y, field: 'B', type: TYPE.NOMINAL },
                        { channel: CHANNEL.COLOR, field: 'C', type: TYPE.QUANTITATIVE, scale: { type: scale_1.ScaleType.POW } },
        ...
        it('should return false for stack specified in incorrect channel', () => {
            NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                ...
                specStackInY.wildcardIndex.setEncodingProperty(0, property_1.Property.STACK, {
                    name: 'stack',
                    enum: wildcard_1.DEFAULT_ENUM_INDEX.stack
                });
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitInvalidStackSpec'].satisfy(specStackInY, schema, config_1.DEFAULT_QUERY_CONFIG));
                ...
                specStackInColor.wildcardIndex.setEncodingProperty(0, property_1.Property.STACK, {
                    name: 'stack',
                    enum: wildcard_1.DEFAULT_ENUM_INDEX.stack
                });
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitInvalidStackSpec'].satisfy(specStackInColor, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
        it('should return true for stack with autoCount.', () => {
            aggregate_1.SUM_OPS.forEach(_ => {
                [CHANNEL.OPACITY, CHANNEL.DETAIL, CHANNEL.COLOR].forEach(stackByChannel => {
                    NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                        const specM = buildSpecQueryModel({
                            mark: MARK.BAR,
    ...
    describe('omitNonSumStack', () => {
        ...
        it('should return true if summative-based aggregate is used.', () => {
            aggregate_1.SUM_OPS.forEach(aggregate => {
                [CHANNEL.OPACITY, CHANNEL.DETAIL, CHANNEL.COLOR].forEach(stackByChannel => {
                    NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                        const specM = buildSpecQueryModel({
                            mark: MARK.BAR,
                        ...
                        specM.wildcardIndex.setEncodingProperty(0, property_1.Property.STACK, { name: 'stack', enum: wildcard_1.DEFAULT_ENUM_INDEX.stack });
                        chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitNonSumStack'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
                    });
                });
            });
        });
            ...
            ['max', 'mean', 'median'].forEach((aggregate) => {
                [CHANNEL.OPACITY, CHANNEL.DETAIL, CHANNEL.COLOR].forEach(stackByChannel => {
                    NON_NULL_STACK_OFFSETS.forEach(stackOffset => {
                        ...
                        specM.wildcardIndex.setEncodingProperty(0, property_1.Property.STACK, { name: 'stack', enum: wildcard_1.DEFAULT_ENUM_INDEX.stack });
                        chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitNonSumStack'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
                    });
                });
            });
        });
    ...
    describe('omitNonPositionalOrFacetOverPositionalChannels', () => {
        it('should return true if only x and y are used', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitNonPositionalOrFacetOverPositionalChannels'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        ...
        it('should return false if color/shape/size is enumerated when either x or y is not used', () => {
            [CHANNEL.SHAPE, CHANNEL.SIZE, CHANNEL.COLOR].forEach(channel => {
                ...
                specM.setEncodingProperty(1, property_1.Property.CHANNEL, channel, { enum: [CHANNEL.SHAPE, CHANNEL.SIZE, CHANNEL.COLOR] });
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitNonPositionalOrFacetOverPositionalChannels'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
            });
        });
        it('should return false if color/shape/size is manually specified if either x or y is not used and we constraintManuallySpecifiedValue', () => {
            [CHANNEL.SHAPE, CHANNEL.SIZE, CHANNEL.COLOR].forEach(channel => {
                ...
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitNonPositionalOrFacetOverPositionalChannels'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
            });
        });
        ...
        it('should return false if facet is enumerated when x or y is not used', () => {
            ...
            specM.setEncodingProperty(1, property_1.Property.CHANNEL, CHANNEL.ROW, { enum: [CHANNEL.ROW] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitNonPositionalOrFacetOverPositionalChannels'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
    describe('omitRaw', () => {
        it('should return false if there is raw data', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRaw'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('It should return true if there is a wildcard aggregate', () => {
            const specM = buildSpecQueryModel({
                mark: MARK.POINT,
                encodings: [{ aggregate: wildcard_1.SHORT_WILDCARD, channel: CHANNEL.X, field: 'A', type: TYPE.QUANTITATIVE }]
            });
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitRaw'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('It should return true if there is a wildcard autoCount', () => {
            const specM = buildSpecQueryModel({
                mark: MARK.POINT,
                encodings: [{ autoCount: wildcard_1.SHORT_WILDCARD, channel: CHANNEL.X, field: 'A', type: TYPE.QUANTITATIVE }]
            });
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitRaw'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    ...
    describe('omitRawContinuousFieldForAggregatePlot', () => {
        it('should return false if the aggregate plot groups by a raw temporal field with timeUnit enumerated as undefined', () => {
            ...
            specM.setEncodingProperty(1, property_1.Property.TIMEUNIT, undefined, { enum: [undefined] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRawContinuousFieldForAggregatePlot'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false if the aggregate plot groups by a raw temporal field with specified timeUnit = undefined', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitRawContinuousFieldForAggregatePlot'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false if the aggregate plot groups by a raw temporal field with specified timeUnit = undefined and we constraintManuallySpecifiedValue', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRawContinuousFieldForAggregatePlot'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
        });
        ...
        it('should return false if the aggregate plot groups by a raw quantitative field that is enumerated', () => {
            ...
            specM.setEncodingProperty(1, property_1.Property.AGGREGATE, undefined, { enum: [undefined] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRawContinuousFieldForAggregatePlot'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    ...
    describe('omitRawDetail', () => {
        it('should return true when raw data does not have the detail channel', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitRawDetail'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false when raw data has an enumerated detail channel', () => {
            ...
            specM.setEncodingProperty(0, property_1.Property.CHANNEL, CHANNEL.DETAIL, { enum: [CHANNEL.DETAIL] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRawDetail'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        ...
        it('should return false when we constraintManuallySpecifiedValue raw data has a manually specified detail channel', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRawDetail'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
        });
    ...
    describe('omitRepeatedField', () => {
        it('should return true when there is no repeated field', function () {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitRepeatedField'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        ...
        it('should return false when repeated fields are not enumerated but constraintManuallySpecifiedValue=true', function () {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRepeatedField'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
        });
        it('should return false when one of the repeated fields is enumerated', function () {
            ...
            specM.setEncodingProperty(1, property_1.Property.FIELD, 'A', { enum: ['A', 'B'] });
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitRepeatedField'].satisfy(specM, schema, CONSTRAINT_MANUALLY_SPECIFIED_CONFIG));
        });
    });
    describe('omitTableWithOcclusionIfAutoAddCount', () => {
        it('return false for raw plot with both x and y as dimensions without autocount or aggregation.', () => {
            [MARK.POINT, MARK.CIRCLE, MARK.SQUARE, MARK.LINE, MARK.AREA, MARK.BAR].forEach(mark => {
                ...
                chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitTableWithOcclusionIfAutoAddCount'].satisfy(specM, schema, { autoAddCount: true }));
            });
        });
        ...
        it('return true for plot with x and y as dimensions and aggregation', () => {
            [MARK.POINT, MARK.CIRCLE, MARK.SQUARE, MARK.LINE, MARK.AREA, MARK.BAR].forEach(mark => {
                ...
                chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitTableWithOcclusionIfAutoAddCount'].satisfy(specM, schema, { autoAddCount: true }));
            });
        });
    ...
    describe('omitVerticalDotPlot', () => {
        it('should return true for horizontal dot plot', () => {
            ...
            chai_1.assert.isTrue(spec_1.SPEC_CONSTRAINT_INDEX['omitVerticalDotPlot'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
        it('should return false for vertical dot plot', () => {
            ...
            chai_1.assert.isFalse(spec_1.SPEC_CONSTRAINT_INDEX['omitVerticalDotPlot'].satisfy(specM, schema, config_1.DEFAULT_QUERY_CONFIG));
        });
    });
});
